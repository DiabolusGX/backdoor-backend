            ...
            switch (action) {
                case "like":
                    downVotes = downVotes.filter(
                        (voter: IUser) =>
                            voter.toString() != user._id.toString()
                    );
                    if (post?.votes.includes(user._id)) {
                        User.findOneAndUpdate(
                            { _id: user._id },
                            { $pull: { votedPosts: post._id } }
                        );
                        votes = post?.votes.filter(
                            (voter) => voter.toString() != user._id.toString()
                        );
                    } else if (post) {
                        User.findOneAndUpdate(
                            { _id: user._id },
                            { $push: { votedPosts: post._id } }
                        );
                        votes.push(user._id);
                    }
                    break;
...
export const updatePost = async (req: Request, res: Response) => {
    ...
    if (updatedPost.user.toString() !== user._id.toString())
        return res
            .status(401)
            .json({ message: "This post is not made by logged in user." });
    ...
    return Post.findById(id)
        .then(async (currPost) => {
            await Post.findOneAndUpdate(
                ...
                const addThread = newPost?.tags.filter(
                    (tag) => !currPost?.tags.includes(tag)
                );
                const removeThread = currPost?.tags.filter(
                    (tag) => !newPost?.tags.includes(tag)
                );
                addThread?.forEach(async (tag) =>
                    Thread.updateOne(
                        { title: tag },
                        {
                            $push: { posts: newPost?._id },
                ...
                removeThread?.forEach(async (tag) =>
                    Thread.updateOne(
                        { title: tag },
                        {
    ...
    return Post.findOne({ _id: id })
        .then(async (post) => {
            if (
                ...
                return res
                    .status(401)
                    .json({ message: "You can not delete this post." });
        ...
        .catch((err) => {
            console.log(err);
            res.status(409).json({
                message: "There was an error while deleting the post.",
            });
        });
};
...
export const searchPost = async (req: Request, res: Response) => {
    ...
    // title and tags
        ...
        dbQuery = {
            $and: [{ title: titleRegexp }, { tags: { $in: tags.split(",") } }],
        };
    // title and user id
    else if (!tags)
    ...
    // tags and user id
        ...
        dbQuery = {
            $and: [{ tags: { $in: tags.split(",") } }, { user: userId }],
        };
    // all 3 given
    else
        dbQuery = {
