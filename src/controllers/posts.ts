...
export const deletePost = async (req: Request, res: Response) => {
    ...
    await Post
        ...
        .catch(err => {
            console.log(err);
            return res.status(409).json({ message: "There was an error while deleting the post." })
        });
}
...
export const searchPost = async (req: Request, res: Response) => {
    ...
    await Post
        ...
        .catch(err => {
            console.error(err);
            return res.status(409).json({ message: "There was an error while fetching the post." });
        });
}